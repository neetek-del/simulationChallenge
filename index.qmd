...
# Define W0 and a simple color palette if not defined in a previous cell
try:
    W0
except NameError:
    W0 = 30000.0
...
try:
    ...
    PALETTE = {"accent2": "steelblue", "accent": "darkred", "highlight": "orangered", "soft": "bisque"}
...
for t in range(1, N+1):
    if np.random.rand() < 0.5: # Replaced undefined 'RNG.random()' with 'np.random.rand()'
        W.append(W[-1] * 1.5)
    else:
        W.append(W[-1] * 0.6)

plt.figure(figsize=(10,4.2))
plt.plot(range(N+1), W, marker='o', linewidth=2, label='Single Simulation Path', color=PALETTE["accent2"])
plt.axhline(W0, color=PALETTE["accent"], linestyle='--', linewidth=2, label="Initial $30k")
...
plt.annotate(f"Final: ${W[-1]:,.0f}", xy=(N, W[-1]), xytext=(N-6, max(W)*0.7),
             arrowprops=dict(arrowstyle="->", color=PALETTE["highlight"]), bbox=dict(boxstyle="round,pad=0.3", fc=PALETTE["soft"]))
plt.tight_layout()
plt.show()

print(f"Starting balance: ${W0:,.2f}")
print(f"Final balance after {N} flips: ${W[-1]:,.2f}")
...
The probability of ending with more than $30,000—the amount initially invested—is quite low. This means that most players would not be happy with the outcome, as their investment decreases over time in most cases.
...
#| fig-cap: "Histogram of final balances across 100 simulations (Ocean Blue gradient)"
#| echo: true
...
for s in range(sims):
    Wt = W0
    for t in range(N):
        if np.random.rand() < 0.5: # Corrected from undefined 'RNG.random()'
            Wt *= 1.5
        else:
            Wt *= 0.6
    finals.append(Wt)
...
prob_above_30k = np.mean(finals > W0)

print(f"Mean final balance: ${mean_final:,.2f}")
print(f"Median final balance: ${median_final:,.2f}")
print(f"P(final > $30k): {prob_above_30k:.3f}")

# Histogram with Ocean blue gradient

plt.figure(figsize=(10,4.5))
# Use 'auto' to let matplotlib select an optimal number of bins
counts, bins, patches = plt.hist(finals, bins='auto', edgecolor='black', alpha=0.9)

# Color gradient for the bars
maxh = counts.max()
for patch, h in zip(patches, counts):
    intensity = 0.3 + 0.7 * (h / maxh)
    patch.set_facecolor((0.0, 0.45 * intensity + 0.1, 0.65 * intensity + 0.15))

plt.axvline(W0, color=PALETTE["highlight"], linestyle='--', linewidth=2, label='Initial $30k')
plt.axvline(mean_final, color="#023E58", linestyle='-', linewidth=2, label='Mean final')
plt.axvline(median_final, color="#2AB7CA", linestyle=':', linewidth=2, label='Median final')
plt.title("Distribution of Final Balances (100 simulations, 40 years)")
