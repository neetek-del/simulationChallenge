...
That’s roughly a 5% gain, which sounds pretty good at first.
...
Colored Lines – Individual Player Paths
...
Black Line – Expected Value Trend
...
The vertical axis displays the players’ account balances in dollars. This scale makes it easy to compare how high or low each player’s balance goes as the simulation unfolds.
...
At the beginning, the balance jumps up a few times, even reaching over $60,000, but the growth doesn’t last long.
...
-A few rare cases show huge gains, reaching into the millions — these outliers pull up the average (mean).

-The median is well below $30,000, showing that more than half of investors would lose money.

-Overall, this is a high-risk, high-reward game — a small chance of big wins, but a high chance of losses.
...
# 🔧 Temporary placeholders if not already defined
# Remove these lines if you already have real simulation data
try:
    finals
except NameError:
    np.random.seed(0)
    finals = np.random.lognormal(mean=10.2, sigma=0.5, size=100) * 1000  # fake data
    W0 = 30000
...
plt.xlabel("Final Account Balance ($ k, log scale)", fontsize=13)
...
-The chart shows the likelihood of ending with more or less than the initial $30,000 after 40 years of investing.

-About 78% of simulations ended with $30,000 or less, meaning most players lost money over time.

-Only about 21% of simulations resulted in a final balance above $30,000, showing that gains were rare.

-This reinforces that the investment is high-risk and unfavorable for most participants, with a small chance of big rewards but a high probability of loss.
...
It significantly reduces the likelihood of catastrophic loss and offers smoother, more predictable growth — but at the cost of missing out on the rare, massive wins seen in the full-bet strategy.
In essence, it’s a clear demonstration of the risk–return trade-off: less risk brings steadier but smaller rewards.
...
Kelly-Optimal Fraction (f*) ≈ 0.28:
The dashed yellow line marks the point where expected log-growth is highest. This represents the mathematically optimal risk level — betting this fraction maximizes long-run wealth without taking excessive risk.
...
Summary:
The Kelly curve confirms that the 25% modified betting strategy is nearly optimal — it achieves strong long-term returns while keeping risk exposure under control. This balance makes it a more sustainable choice compared to full-bet or over-leveraged strategies.
